...
def main():
    ...
    if not os.path.isfile(palette_path):
        print(f"Error: palette2.py not found in {script_dir}")
        return
    if not os.path.isfile(hgpt_path):
        print(f"Error: hgpt.py not found in {script_dir}")
        return
    if not os.path.isfile(hgar_path):
        print(f"Error: hgar.py not found in {script_dir}")
        return
    ...
    # --- Step 3: Prepare input for hgpt ---
    if png_file.endswith(".hpt.DECOMPRESSED.PICTURE.png"):
        hgpt_input = png_file
    else:
        hgpt_input = os.path.splitext(png_file)[0] + ".hpt.DECOMPRESSED.PICTURE.png"
    ...
    base_name_no_ext = os.path.basename(hgpt_input).replace(".hpt.DECOMPRESSED.PICTURE.png", "")
    hgpt_output = os.path.join(folder, f"{base_name_no_ext}.hpt.DECOMPRESSED")

    if not os.path.isfile(hgpt_output):
        print(f"Error: Expected hgpt output not found: {hgpt_output}")
        return

    # --- Step 6: Rename to .hpt and move to HAR folder ---
    final_name = f"{base_name_no_ext}.hpt"
    har_folder = os.path.dirname(os.path.abspath(har_file))
    target_path = os.path.join(har_folder, final_name)

    if os.path.exists(target_path):
        os.remove(target_path)

    shutil.move(hgpt_output, target_path)
    print(f"Moved {hgpt_output} -> {target_path} (overwritten if existed)")
